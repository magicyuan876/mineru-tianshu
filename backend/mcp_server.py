...
- 异步处理，可选择等待完成或稍后查询
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "file_base64": {
                        "type": "string",
                        "description": "Base64 编码的文件内容（适用于 < 100MB 的文件）"
                    },
                    "file_name": {
                        "type": "string",
                        "description": "文件名（使用 file_base64 时必需）"
                    },
                    
                    # 方式 2: URL 下载
                    "file_url": {
                        "type": "string",
                        "description": "文件的公网 URL（服务器会自动下载）"
                    },
...
- 解析结果内容（如果完成）
            ...
            inputSchema={
                ...
                "properties": {
                    "task_id": {
                        "type": "string",
                        "description": "任务 ID（由 parse_document 返回）"
                    },
                    "include_content": {
                        "type": "boolean",
            ...
            if "file_base64" in args:
                ...
                if size_mb > 100:
                    return [TextContent(
                        type="text",
                        text=json.dumps({
                            "error": f"File too large ({size_mb:.1f}MB). Maximum size is 100MB."
                        }, indent=2)
                    )]
            ...
            # 方式 2: URL 下载
                ...
                try:
                    ...
                    return [TextContent(
                        type="text",
                        text=json.dumps({
                            "error": f"Timeout downloading file from {url}"
                        }, indent=2)
                    )]
                except Exception as e:
                    return [TextContent(
                ...
                return [TextContent(
                    type="text",
                    text=json.dumps({
                        "error": "Must provide either file_base64 or file_url"
                    }, indent=2)
                )]
