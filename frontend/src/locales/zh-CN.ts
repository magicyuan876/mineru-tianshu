...
export default {
  ...
  task: {
    ...
    startedAt: '开始时间',
    timeInfo: '时间信息',
    // 主子任务分组（大 PDF 拆分 / ZIP 解包）
    subtaskCount: '{total} 个分片',
    subtaskCompleted: '完成 {completed}/{total}',
    subtaskFailed: '失败 {count}',
    subtaskRunning: '处理中 {count}',
    expandSubtasks: '展开分片',
    collapseSubtasks: '收起分片',
    loadingSubtasks: '加载分片中…',
    subtaskLoadError: '加载分片失败',
    subtaskPages: '第 {start}–{end} 页',
    subtaskEntry: '#{index} {name}',
    confirmRetryParent: '将重新解析 {count} 个失败或已取消的分片，已完成的分片保留，全部完成后自动合并结果。确定重试吗？',
    confirmRetryParentMergeOnly: '所有分片均已完成，将重新合并解析结果。确定重试吗？',
    confirmCancelParent: '确定取消该任务吗？未完成的分片会一并取消，任务记录会保留。',
    processingTime: '处理时长',
    actions: '操作',
    viewDetail: '查看详情',
    deleteTask: '删除任务',
  ...
  systemConfig: {
    ...
    nav: {
      ...
      taskConfig: '任务处理',
      taskConfigDesc: '任务卡在处理中超时后的自动重试策略',
      auditLog: '审计日志',
  ...
  taskConfig: {
    title: '任务处理',
    description: '调度器会定期检查长时间停留在「处理中」的任务（默认超过 60 分钟），按以下策略处理',
    maxRetries: '超时自动重试次数',
    maxRetriesHelp: '任务超时后自动重新排队的次数，用完后判定失败并记录原因；0 表示超时即失败。手动重试会重新获得完整次数。取值 0–{max}',
    invalid: '请输入 0–{max} 之间的整数',
    loadError: '加载任务处理配置失败',
    saving: '保存中...',
    saveSuccess: '配置保存成功',
    saveError: '配置保存失败',
  },
