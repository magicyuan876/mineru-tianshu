...
export default {
  ...
  webhook: {
    title: 'Webhook 通知',
    description: '任务进入终态（完成/失败）时，向配置的回调地址推送签名通知（仅管理员）',
    enabled: '启用 Webhook 通知',
    enabledHelp: '启用后，任务完成或失败时会向回调地址 POST 一条带 HMAC 签名的 JSON 通知',
    url: '回调地址（URL）',
    ...
    authSecretPlaceholder: '已配置，留空则不修改',
    eventCompleted: '任务完成 (task.completed)',
    eventFailed: '任务失败 (task.failed)',
    timeout: '超时（秒）',
    maxAttempts: '最大重试次数',
